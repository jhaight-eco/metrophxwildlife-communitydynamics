...
#### Import Model Output File ####
  ...
  # out <- readRDS("./data/model_outputs/globalDCM_sample3k.rds")   
  out <- readRDS("./data/model_outputs/globalDCM_sample30k.rds")
# 
...
#### Plot Model Results ####
  ...
  # all points should be above a 1-1 line, given that "presence" has been adjusted for imperfect detection
...
#### Fit Species Richness Model #####
  ...
  # We can (and need to) up the amount of posterior samples to ensure convergence here
  ...
  # view the summary
  m.sr.sum %>% as.data.frame() %>% filter(Rhat < 1.05)    # confirm that all the parameters converged
  m.sr.sum %>% as.data.frame() %>% filter(Rhat >= 1.05)   # there should be nothing here
  
  saveRDS(m.sr, "./data/model_outputs/metaanalysislogglm_speciesrichness_90k.rds")
      # previous versions of the model with fewer iterations
      ## saveRDS(m.sr, "./data/model_outputs/metaanalysislogglm_speciesrichness_30k.rds")
      ## saveRDS(m.sr, "./data/model_outputs/metaanalysislogglm_speciesrichness_3k.rds")
