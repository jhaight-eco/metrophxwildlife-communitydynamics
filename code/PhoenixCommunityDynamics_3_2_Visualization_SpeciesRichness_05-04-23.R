...
setwd("C:/Users/User/Documents/GitHub/metrophxwildlife-communitydynamics")
  ...
  saguaro <- park_palette("Saguaro")
  load("G:/My Drive/Presentations_Figures/Figure Materials/color palettes/arizonapalettes.RData")
...
##### Import Data #####
  ...
  m.sr <- readRDS("./data/5_output/metaanalysis/metaanalysislogglm_speciesrichness_30k_05-04-2023.rds")
  
  m.sr.sum <- read.csv("./data/5_output/metaanalysis/communitymetaanalysisresults_richness_05-04-2023.csv")
  ...
  # vegetation greenness had been removed
  #colnames(m.sr.mcmc.s1) <- c("int", "urb","urbquad", "het", "veg", "re", "dev")
  #colnames(m.sr.mcmc.s2) <- c("int", "urb","urbquad", "het", "veg", "re", "dev")
  #colnames(m.sr.mcmc.s3) <- c("int", "urb","urbquad", "het", "veg", "re", "dev")
  colnames(m.sr.mcmc.s1) <- c("int", "urb","urbquad", "het", "re", "dev")
  colnames(m.sr.mcmc.s2) <- c("int", "urb","urbquad", "het", "re", "dev")
  colnames(m.sr.mcmc.s3) <- c("int", "urb","urbquad", "het", "re", "dev")
  ...
  # check where the peak/valley of the urbanization curve would be in each season
  #(mean(m.sr.mcmc.s1[,2])/mean(m.sr.mcmc.s1[,3])*-1)*sd(data$imp1km)+mean(data$imp1km)  # very, very, very slightly concave
  #(mean(m.sr.mcmc.s2[,2])/mean(m.sr.mcmc.s2[,3])*-1)*sd(data$imp1km)+mean(data$imp1km)  # very, very slightly convex
  #(mean(m.sr.mcmc.s3[,2])/mean(m.sr.mcmc.s3[,3])*-1)*sd(data$imp1km)+mean(data$imp1km)  # very slightly convex, peaking at negative levels of urbanization
...
# 2) Predict across Patch Diversity Gradient
    ...
    preds.sr.het1 <- exp(m.sr.mcmc.s1[,1:ncol(dm.het)] %*% t(dm.het))   # exponentiate, because that's the reverse of the log-link
    preds.sr.het1 <- apply(
      preds.sr.het1,
      2,
      quantile, 
    ...
    colnames(preds.sr.het1)[1:3] <- c("lower95", "median", "upper95")
    # this is not a season-varying covariate, but the 
    
    # add a column for the covariate
    preds.sr.het1$sdhi <- seq(min(cov3), max(cov3), length.out = npred)
    preds.sr.het1$sdhi_std <- seq(min(cov3.std), max(cov3.std), length.out = npred)
    str(preds.sr.het1)
    
    # repeat for each season, using the different betas in each season
    # Season 2: Warm-Wet
    preds.sr.het2 <- exp(m.sr.mcmc.s2[,1:ncol(dm.het)] %*% t(dm.het))   # note that it is now m.sr.mcmc.s2
    preds.sr.het2 <- apply(
      preds.sr.het2,
      2,
      quantile, 
    ...
    colnames(preds.sr.het2)[1:3] <- c("lower95", "median", "upper95")
    
    preds.sr.het2$sdhi <- seq(min(cov3), max(cov3), length.out = npred)
    preds.sr.het2$sdhi_std <- seq(min(cov3.std), max(cov3.std), length.out = npred)
    str(preds.sr.het2)
    
    # Season 3: Cool-Wet
    preds.sr.het3 <- exp(m.sr.mcmc.s3[,1:ncol(dm.het)] %*% t(dm.het))   # m.sr.mcmc.s3
    preds.sr.het3 <- apply(
      preds.sr.het3,
      2,
      quantile, 
    ...
    colnames(preds.sr.het3)[1:3] <- c("lower95", "median", "upper95")
    
    preds.sr.het3$sdhi <- seq(min(cov3), max(cov3), length.out = npred)
    preds.sr.het3$sdhi_std <- seq(min(cov3.std), max(cov3.std), length.out = npred)
    str(preds.sr.het3)
