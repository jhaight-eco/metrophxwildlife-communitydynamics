...
#### Import Data ####
  ...
  out <- readRDS("C:/Research/urban/CAPLTER/projects/MetroPhoenixWildlifeStudy/CH1_communitydynamics/analysis/data/5_output/DCM_global/globalDCM_sample30k.rds")    
...
# Where do the effects of urbanization peak?
  ...
  # standardized values of imperviousness
    (peak.psi <- -1*mean(tmp$mu.beta1.psi)/(2*mean(tmp$mu.beta2.psi)))
    (peak.phi <- -1*mean(tmp$mu.beta1.phi)/(2*mean(tmp$mu.beta2.phi)))
    (peak.gamma <- -1*mean(tmp$mu.beta1.gamma)/(2*mean(tmp$mu.beta2.gamma)))
  # de-standardized them
    (peak.psi)*sd(data.site$imp1km)+mean(data.site$imp1km)
    # there is a substantial quadratic term for urbanization, but
    ...
    # and the peak is at ~16.4% urbanization
```
...
# BONUS
```{r NMDS based on occupancy}
str(tmp$psi)
psi.mean <- apply(tmp$psi, MARGIN = c(2, 3, 4), function(x) mean(x, na.rm = TRUE))
str(psi.mean)

# reshape the occupancy data (the mean estimates of psi)
spe <- rbind(
    psi.mean[,1,],
    psi.mean[,2,],
    psi.mean[,3,]
...
rownames(spe) <- c(
  paste(data.site$site, "1", sep = "_"),
  paste(data.site$site, "2", sep = "_"),
  paste(data.site$site, "3", sep = "_")
)
colnames(spe) <- data.spp$Species


# reshape the environmental data
env <- data.frame(
  "site" = rep(data.site$site, n.season),
  "season" = rep(c("1_wd", "2_ww", "3_cw"), each = nrow(data.site)),
  "impervious" = rep(data.site$imp1km, n.season),
  "pd" = rep(data.site$pd5km, n.season),
  "ndvi" = c(data.site$ndvi_wd1km, data.site$ndvi_ww1km, data.site$ndvi_cw1km)
  )
...
# env.z <- decostand(env, method = "standardize")  # decostand can be used to standardize environment variables as well
```
```{r}
nmds.psi <- metaMDS(spe[,which(!colnames(spe) %in% c(
    "MEME"
  ))], k=3, trymax=100) 
ordiplot(nmds.psi, type="n")     # plot again without the basic points
    ordiellipse(nmds.psi, groups=env$season, draw="lines", col=c("blue","red","black"), label=F, kind = "ehull", 
                lwd = 2, lty = c(1, 2, 3))  # add elliipses
    # orditorp(nmds.psi, display="species", col="orange", cex=1, air=0.01)   # add in the species points
    orditorp(nmds.psi, display="sites", cex=1, air=0.01)      # add in the individual sites
```

```{r}

# env <- data1 %>% arrange(site_code, survey_year) %>% 
#   select(
#     "NDBI",
#     "NDISI",
#     "ENDISI",
...
# dist(x = spe, method = "euclidean")
# vegdist(x = spe, method = "chord")
vegdist(spe, method = "bray", binary = FALSE)
vegdist(spe[c(1,51),], method = "bray", binary = FALSE)

# try transforming the data using 'decostand'
spe.pa <- decostand(spe, method = "pa")   # transform to presence-absence (0-1)
spe.pa[1:6, 1:6]
#spe.total <- decostand(spe, method = "hellinger") # Hellinger tranformation = square-root of relative abundance


```
```{r try running a PCA}
# code from https://r.qcbs.ca/workshop09/book-en/principal-component-analysis.html
# First, the long-form method (13.1)
    data <- env %>% data.frame %>% select(-c()) %>% as.matrix()      # select all the environmental variables 
    data <- decostand(spe, method = "hellinger")
    data_std <- scale(data)  # scale the data
    cov_matrix <- cov(data_std)  # compute covariance matrix
    eigen_decomp <- eigen(cov_matrix)   # perform the Eigendecomposition of the covariance matrix
    ...
    F_PrComps <- data_std %*% Eigenvectors   # project the standardized data onto the Eigenspace
    head(F_PrComps)

# Second, try using some package functions
pca <- prcomp(data, center = TRUE, scale = TRUE) #%>% summary()
# pca.env <- vegan::rda(data_std)# %>% summary()
pca <- vegan::rda(data_std)
# ev <- pca$CA$eig
# ev[ev > mean(ev)]
summary(pca)
biplot(pca)
